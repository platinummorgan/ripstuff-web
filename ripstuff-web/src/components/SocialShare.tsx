...
export function SocialShare({ 
  ...
  const shareLinks = {
    ...
    facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}&quote=${encodedTitle}${description ? ` - ${encodedDescription}` : ''}`,
    reddit: `https://reddit.com/submit?url=${encodedUrl}&title=${encodedTitle}${description ? ` - ${encodedDescription}` : ''}`,
  };
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 text-sm text-[var(--muted)] hover:text-[var(--accent)] transition-colors"
      ...
      {showDropdown && (
        <div className="absolute right-0 mt-2 w-48 bg-[#0B1123] border border-[rgba(255,255,255,0.08)] rounded-lg shadow-xl z-50">
          <div className="p-2">
            <div className="text-xs text-[var(--muted)] mb-2 px-2">Share this memorial</div>
